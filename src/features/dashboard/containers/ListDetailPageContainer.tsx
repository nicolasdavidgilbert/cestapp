...
export default function ListDetailPage() {
  ...
  const publishListEvent = useCallback(
    async (eventName: string, payload: RealtimeEventPayload) => {
      if (!user) return
      await publishListRealtimeEvent(listChannel, eventName, {
        ...payload,
        list_id: listId,
        user_id: user.id,
  ...
  const publishUserListsEvent = useCallback(
    async (targetUserId: string, action: string) => {
      ...
      await publishUserListsRealtimeEvent(targetUserId, listId, action, user?.id)
    },
    [listId, user?.id]
  )
  ...
  async function addMemberByEmail() {
    ...
    const { data, error } = await shareListWithEmail(listId, targetEmail)
  ...
  async function createInviteLink() {
    ...
    setError('')
    const { data, error } = await createInviteLinkRecord(listId, user.id, getInviteExpiryDate(inviteExpiry))
  ...
  async function revokeInviteLink(invite: InviteLink) {
    ...
    setError('')
    const { error } = await revokeInviteLinkRecord(invite.id)
  ...
  async function updateListName() {
    ...
    setError('')
    const { data, error } = await updateShoppingListName(list.id, nextName)
  ...
  async function removeMember(member: ShoppingListShare) {
    ...
    setError('')
    const { error } = await deleteListShare(member.id)
  ...
  async function createProductWithValues(titleInput: string, descriptionInput: string, priceInput: string) {
    ...
    const { data, error } = await createProductForList(listId, title, description, price)
    ...
    const [productRes, itemRes] = await Promise.all([
      fetchListProductSummary(created.created_id),
  ...
  const {
    ...
    filteredProducts,
    suggestedProducts,
  } = useListDerivedState({ items, products, productSearch })
      ...
      <div className="mx-auto w-full max-w-4xl space-y-6">
        <header className="space-y-4">
          ...
          <div className="flex w-full items-center gap-1 rounded-2xl border border-border bg-muted/20 p-1.5 backdrop-blur-md overflow-x-auto sm:overflow-x-visible">
            ...
            ] as DashboardTabDefinition[]).map((tab) => (
              <button
                key={tab.id}
